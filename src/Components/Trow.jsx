...
const Trow = ({log}) => {
  ...
  <tr className='odd:bg-neutral-300 even:bg-neutral-200'>
    ...
    <td className="pl-4">{(log.person.firstName + " "+log.person.lastName).length > 14 && (log.person.firstName + ""+log.person.lastName) != "Unknown Intruder" ? (log.person.firstName + " "+log.person.lastName).substring(0, 12)+"...": (log.person.firstName + " "+log.person.lastName) }</td>
